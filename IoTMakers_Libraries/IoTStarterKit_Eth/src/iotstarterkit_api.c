...
static unsigned long __remove_redundancy() // Remove Needless Packet
{
	unsigned long pktLen;

	if ( im_sock_recv_add((char*)&pktLen, 4) < 0 )	{
		return -1;
	};
	return 0;
}
...
int im_auth_device() // Temporarily
{
	...
	// Temporarily
	__recv_packet(IM_READ_TIMEOUT_SEC); // For Debug
 ...
 * im_send_numdata
 */
int im_send_numdata(const char* tagid, double val, long long trxid) // Temporarily
{
	unsigned int sent = 0;
