...
const unsigned char head_03[] PROGMEM = {
  0x00, 0x00, 0x01, 0x4e, 0x05, 0x5f, 0xdf, 0xce, // Transactio ID // 8
};
...
const unsigned char head_05[] PROGMEM = {
  ...
  0x00, // Compression Usage(1bit) + Compression method(7bit) // 5
  0x03, // Encoding Type
  0x00, 0x00 // Result Code
};
// Wow ReDefined ///////////////////////////////////////////////////////////////////////
const unsigned char head_06[] PROGMEM = { // Auth
  0x11, // Protocol Version
  0x01, // Message Header Type
  0x00, 0x23, // Message header Length 0x23 = 35
  0x60, 0xe0, // Message Type(2bit) + MEP(2bit) + Method Type(12bit)
...
const unsigned char head_07[] PROGMEM = { // Send Collection Report
  0x11, // Protocol Version
  0x01, // Message Header Type
  0x00, 0x23, // Message header Length 0x23 = 35
  0x61, 0x9b, // Message Type(2bit) + MEP(2bit) + Method Type(12bit)
...
const unsigned char head_08[] PROGMEM = { // Keep Alive
  0x11, // Protocol Version
  0x01, // Message Header Type
  0x00, 0x23, // Message header Length 0x23 = 35
  0x60, 0xe7, // Message Type(2bit) + MEP(2bit) + Method Type(12bit)
...
const unsigned char head_09[] PROGMEM = { // Keep Alive
  0x11, // Protocol Version
  0x01, // Message Header Type
  0x00, 0x23, // Message header Length 0x23 = 35
  0x60, 0xe7, // Message Type(2bit) + MEP(2bit) + Method Type(12bit)
...
int __head_send_P(char* PROGMEM data, int len) // Temporarily
{
...
{
	int sent = 0;
	const unsigned char head_06_R[40] = { // Auth
	  0x11, // Protocol Version
	  0x01, // Message Header Type
	  0x00, 0x23, // Message header Length 0x23 = 35
	  0x60, 0xe0, // Message Type(2bit) + MEP(2bit) + Method Type(12bit)
	...
	#if defined(WOW_USART)
		sent += im_sock_send(head_06_R,35);
	#else
...
int head_send_keepalive() // Temporarily
{
	int sent = 0;
...
int head_send_control_ack(char* authCode)
{
	int sent = 0;
		...
		sent += __head_send_P((char*)head_03, sizeof(head_03));
		sent += im_sock_send((char*)authCode, 16);
		sent += __head_send_P((char*)head_05, sizeof(head_05));
	#else
		sent += __head_send_P((char*)head_01, sizeof(head_01));
		sent += __head_send_P((char*)head_02_TypeAck, sizeof(head_02_TypeAck));
